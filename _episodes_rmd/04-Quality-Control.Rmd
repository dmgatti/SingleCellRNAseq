...
As mentioned in the previous lesson, the data we are working with
contains `r sum(metadata$typeSample == 'scRnaSeq')` single *cell*
transcriptomes and `r sum(metadata$typeSample == 'snRnaSeq')`
single *nucleus* transcriptomes.
Capturing transcripts from whole cells is very different from
capturing only those that are in the nucleus. Thus, the data
properties defining each data type are different.
Therefore, we should perform quality control separately on 
each of the two data types.
...
of any of the other genes and we could remove them before proceeding with 
further analysis.
...
we just resized the counts matrix, we need to recalculate the gene_counts.
