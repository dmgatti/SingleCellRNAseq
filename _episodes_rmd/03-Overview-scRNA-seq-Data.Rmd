...
Call Ranger also produces a Quality Control (QC) report as a PDF. We do not have
the QC report from the Liver Atlas study, but the figure below shows an 
example report. The report highlights three numbers:

> DAS: Is there one report per chip or one report per channel?
> DAS: Check text below.

1. Estimated Number of Cells: This indicates the number of cells in your 
experiment. Ideally, this number should be between ??? and ???.
1. Mean Reads per Cell: This indicates the number of reads in each cell. 
Ideally this number should be between ??? and ???.
1. Median Genes per Cell: This indicates the median number of genes detected
in each cell. Note that this is much lower than in bulk RNA-Seq.
