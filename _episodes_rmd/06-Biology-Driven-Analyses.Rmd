...
```{r labelling2,echo=FALSE}
# We need this block to fill in the cell types from the challenge above.
labels <- tibble(cluster_num = unique(liver$labels)) %>%
          mutate(cluster_num = as.character(cluster_num)) %>%
          mutate(cluster_name = case_when(
                 cluster_num == 'c5'  ~ 'monocytes',
                 cluster_num == 'c10' ~ 'fibroblast/stellate',
                 cluster_num == 'c11' ~ 'cCD1',
